...
OPCClient* OPCClient::m_pInstance = NULL;
...
{
    m_Async = async;      //异步同步读写
    m_GroupName = qstr2Wstr(n);
    m_ServerHandle = 0;
    //m_UpdateRate = 100;
    m_UpdateRate = 10;    //刷新率20ms
    long TimeBias = 0;
    DWORD LanguageCode = 0x409; //0x416;
    ...
    {
        // qstrTemp = "1004:请求的刷新速率与实际的刷新速率不一致";
        qDebug()<< "1004:";
    }
    ...
    if( FAILED(hResult)) {
        qDebug()<< "1005: 创建分组失败";
        DisConnect();
        return hResult; //add xgb
    }
    if( _pIOPCGroupStateMgt == NULL) {
        qDebug()<< "1005: 创建分组失败";
        DisConnect();
        return hResult; //add xgb
    }
...
QStringList OPCClient::AddItems(QStringList inames)
{
    ...
    //const int nCount = r_inames.count();//6;
    //int nCount = r_inames.count();
    if(nCount>0){

        OPCITEMDEF *idef = new OPCITEMDEF[nCount];

        //QVector<OPCITEMDEF>  idef(nCount);
        //Vector<OPCITEMDEF>  idef;
        ...
        if(hResult==S_OK && hResult!=S_FALSE){
            qDebug()<<"AddItems ok "<<hResult;
        }else{
            qDebug()<<"AddItems error "<<hResult;
        }
...
* 异步读可以用中断，同步直接扫描
*/
bool OPCClient::SyncReadOPCItem(DWORD rHandle, VARIANT *saveValue)
{

    ItemDef * item = getItemByHandle( rHandle );
    if( !item )
        return false;
    if(!item )
        return false;
...
bool OPCClient::SyncReadOPCItemS(QString &itemName,VARIANT *saveValue)
{

    ItemDef * item = getItemByName( itemName );
    if( !item )
        return false;
    if(!item )
        return false;
...
bool OPCClient::WriteValueS(QString &n,VARENUM writeType, QString &v )
{
    //qDebug()<<"WriteValueS name = "<<n;
    ItemDef * item = getItemByName(n);
    if(!item ){
        qDebug()<<"getItemByName error"<<n;
        return false;
    }
    item->value.vt = writeType;
...
HRESULT OPCClient::WriteValue_Async(ItemDef * item)
{
    ...
    if(item->value.vt == VT_BSTR){ //用了这个字符串，需要释放掉 xgb
        ::SysFreeString(item->value.bstrVal);
    }
...
HRESULT OPCClient::RemoveItems(QStringList inames)
{
    ...
    OPCHANDLE *_hpSvr = new OPCHANDLE[_iCount];
...
/**
 * 获取ItemDef by name    后期如果速度有影响，需要优化这里的list考虑用map或者其他更好的办法
...
void OPCClient::StringVariant(ItemDef* item, QString v)
{
    ...
    case VT_BSTR:
    {
        const OLECHAR* pp = reinterpret_cast<const wchar_t *>(v.utf16());
        item->value.bstrVal =  SysAllocString(pp); //在外部释放的 xgb
